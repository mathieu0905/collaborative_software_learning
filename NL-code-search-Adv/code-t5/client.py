...
class FlowerClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        set_parameters(self.model, parameters)
        loss, mrr = evaluate(args, self.model, self.testset)
        return loss, len(self.testset), {"mrr": mrr}
...
class CustomFedAvg(fl.server.strategy.FedAvg):
    def aggregate_evaluate(
        ...
        mrr = [r.metrics["mrr"] * r.num_examples for _, r in results]
        examples = [r.num_examples for _, r in results]

        # Aggregate and print custom metric
        aggregated_mrr = sum(mrr) / sum(examples)
        print(f"Round {server_round+args.checkpoint_round} mrr aggregated from client results: {aggregated_mrr}")

        # Return aggregated loss and metrics (i.e., aggregated accuracy)
        return aggregated_loss, {"mrr": aggregated_mrr}
    
    def aggregate_fit(self, rnd, results, failures):
        logging.info(f"Server: Aggregating parameters in round {rnd+args.checkpoint_round}")
        aggregated_parameters, aggregated_metrics = super().aggregate_fit(rnd, results, failures)

        if aggregated_parameters is not None:
            print(f"Saving round {rnd+args.checkpoint_round} aggregated_parameters...")
            ...
            torch.save(model.state_dict(), f"{args.output_dir}/model_round_{rnd+args.checkpoint_round}.bin")
