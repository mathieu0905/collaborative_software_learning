...
def eval_acc(args, model, tokenizer, file_type='test'):
    ...
    def DecodeIds(idxs):
        ...
        for idx in idxs:
            ...
            if tokenizer.convert_ids_to_tokens(idx)[0] == '\u0120' or tokenizer.convert_ids_to_tokens(idx)[0] == '▁':
        ...
        for pred, gt in zip(pred_ids, inputs):
            ...
            for i, y in enumerate(gt):
                if i == 0:
                    ...
                    if tokenizer.convert_ids_to_tokens(y)[0] == '\u0120' or tokenizer.convert_ids_to_tokens(y)[0] == '▁':
                        if len(now_gt) > 0:
                            try:
                                all_pred.append(DecodeIds(now_pred).strip().split()[0])
                            except IndexError:
...
def post_process(args, preds, gts, true_gts, saved_file):
    ...
    for i, (pred,gt) in enumerate(zip(preds,gts)):
        ...
        if gt == "</s>":
            ...
            # logging.warning(f"gt_str = {gt_str}, true_gts[cnt].strip() = {true_gts[cnt].strip()}, pred_str = {pred_str}")
...
def main():
    ...
    if pretrained:
        tokenizer = tokenizer_class.from_pretrained(pretrained, do_lower_case=args.do_lower_case, sep_token='<EOL>', bos_token='<s>', eos_token='</s>', pad_token='<pad>', unk_token='<|UNKNOWN|>', additional_special_tokens=special_tokens, cache_dir="/data/model")
        logging.info(tokenizer.sep_token)
        if args.model_type == "rnn":
            ...
            # model = model_class.from_pretrained(pretrained, cache_dir="/data/model")
            # model.resize_token_embeddings(len(tokenizer))
            if args.start_step == 0 and args.do_train:
                model = model_class.from_pretrained(pretrained, cache_dir="/data/model")
            elif args.model_type == "llama":
                from peft import LoraConfig, get_peft_model, PeftModel
                if args.start_step != 0 or args.do_eval:
                    model_name_or_path = "codellama/CodeLlama-7b-hf"  # 基础模型的路径
                    model = model_class.from_pretrained(model_name_or_path, cache_dir="/data/model")
                    if args.do_train:
                        model = PeftModel.from_pretrained(model, pretrained, is_trainable=True)
                    else:
                ...
                model.resize_token_embeddings(len(tokenizer))
                model.load_state_dict(torch.load(os.path.join(args.output_dir, 'model_round_3.bin')), strict=False)
                logger.info("load model from model_round_3.bin")
            model.resize_token_embeddings(len(tokenizer))
        ...
        if args.model_type == "rnn":
            ...
            config = config_class.from_pretrained(args.config_dir, cache_dir="/data/model")
        ...
        # if args.start_step != 0 or args.do_eval:
        #     peft_model = f"{pretrained}/adapter_model.bin"
        #     model.load_state_dict(torch.load(peft_model, map_location="cpu"), strict=False)
