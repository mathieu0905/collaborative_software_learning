...
def load_and_cache_examples(args, tokenizer, cid, evaluate=False):
    if args.not_pretrain:
        dataset = finetuneDataset(tokenizer, args, logger, cid, file_type='dev' if evaluate else 'train', 
                                block_size=args.block_size)
    else:
        dataset = TextDataset(tokenizer, args, logger, cid, file_type='dev' if evaluate else 'train',
                                block_size=args.block_size)
    # 截取前100项
    # dataset = dataset[:100]
    return dataset         
...
def load_data(cid, tokenizer, args):
    train_dataset = load_and_cache_examples(args, tokenizer, cid, evaluate=False)
    eval_dataset = load_and_cache_examples(args, tokenizer, cid, evaluate=True)
...
def load_model(args):
    ...
    if pretrained:
        tokenizer = tokenizer_class.from_pretrained(pretrained, do_lower_case=args.do_lower_case, sep_token='<EOL>', bos_token='<s>', eos_token='</s>', pad_token='<pad>', unk_token='<|UNKNOWN|>', additional_special_tokens=special_tokens, cache_dir=args.cache_dir if args.cache_dir else None)
        if args.model_type == "rnn":
            ...
            model = model_class.from_pretrained(pretrained, cache_dir=args.cache_dir if args.cache_dir else Non)
...
def evaluate(args, model, tokenizer, eval_dataset, logger, prefix="", eval_when_training=False):
    ...
    return eval_loss, float(perplexity)