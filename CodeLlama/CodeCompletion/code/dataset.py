...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, logger, cid=0, file_type='train', block_size=1024):
        ...
        cached_file = os.path.join(args.output_dir, file_type+"_langs_%s"%(args.langs)+"_blocksize_%d"%(block_size)+f"_cid_{cid}"+"_wordsize_%d"%(world_size)+"_rank_%d"%(local_rank))
...
class finetuneDataset(Dataset):
    def __init__(self, tokenizer, args, logger, cid=0, file_type='train', block_size=1024):
        ...
        if not os.path.exists(args.output_dir):
            os.makedirs(args.output_dir)
        cached_file = os.path.join(args.output_dir, file_type+"_blocksize_%d"%(block_size)+f"_cid_{cid}"+"_wordsize_%d"%(world_size)+"_rank_%d"%(local_rank))
        if os.path.exists(cached_file) and not args.overwrite_cache:
            if file_type == 'train':
                logger.warning("Loading features from cached file %s", cached_file)
            with open(cached_file, 'rb') as handle:
            ...
            datafile = os.path.join(args.data_dir, f"{file_type}_{cid}.txt")
            if file_type == 'train':
                logger.warning("Creating features from dataset file at %s", datafile)
            with open(datafile) as f:
                data = f.readlines()
...
class EvalDataset(Dataset):
    def __init__(self, tokenizer, args, logger, file_type='train', block_size=1024):
        ...
        if os.path.exists(cached_file) and not args.overwrite_cache:
            ...
            logger.debug("Saving features into cached file %s", cached_file)
            del input_ids
            gc.collect()
    ...
    def split(self, input_ids, tokenizer, logger, block_size=1024):
        logger.info(f"before split, {len(input_ids)} samples")
        sample = []
        i = 0
        while i < len(input_ids):
            ...
            if len(sample) == block_size:
                for j in range(block_size):
                    #  or tokenizer.convert_ids_to_tokens(sample[block_size-1-j])[0] == '▁'
                    if tokenizer.convert_ids_to_tokens(sample[block_size-1-j])[0] == '\u0120' or tokenizer.convert_ids_to_tokens(sample[block_size-1-j])[0] == '▁' or tokenizer.convert_ids_to_tokens(sample[block_size-1-j]).startswith("<NUM_LIT"):
                        logger.info(i+block_size-1-j)
                        break
        ...
        logger.info("after split, %d samples"%(len(self.inputs)))
        # exit()
    ...
    def __getitem__(self, item):
        if isinstance(item, slice):
            # 处理切片，返回一个数据列表
            return [torch.tensor(self.inputs[i]) for i in range(*item.indices(len(self)))]
        else:
            return torch.tensor(self.inputs[item])
