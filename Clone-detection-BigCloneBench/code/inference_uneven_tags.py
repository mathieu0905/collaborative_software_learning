...
def test(args, model, tokenizer, prefix="",pool=None,best_threshold=0):
    ...
    for batch_index, batch in enumerate(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            # 添加关于损失和模型输出的调试信息
            logger.info(f"Batch {batch_index}: Loss = {lm_loss.mean().item()}")
            logger.info(f"Batch {batch_index}: Logit[0] = {logit[0]}")  # 显示第一个输出的 logit，作为示例
    ...
    # 记录预测结果的摘要信息
    logger.info("Evaluation complete. Writing predictions to file.")
    positive_predictions = np.sum(y_preds)
    logger.info(f"Number of positive predictions: {positive_predictions} / {len(y_preds)}")
...
def main():
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        # checkpoint_prefix = 'aggregated_parameters_round_5.bin'
        # output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  

        with open('saved_models_uneven_tags/aggregated_parameters_round_5.pkl', 'rb') as f:
            aggregated_parameters = pickle.load(f)
