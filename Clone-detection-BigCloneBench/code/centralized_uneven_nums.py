...
def train(args, model, train_dataset, eval_dataset, client_id):
    """ Train the model """
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        eval_loss, results = evaluate(args, model, eval_dataset)                 
                    ...
                    if results['f1']>best_f1:
                        ...
                        output_dir = f'{args.output_dir}/{checkpoint_prefix}/client_{client_id}'
        ...
        logging.info(f"Finished epoch {idx}")                
        if args.max_steps > 0 and global_step > args.max_steps:
            # train_iterator.close()
            break

        # model_save_path = os.path.join(args.output_dir, f"model_client_{client_id}_epoch_{idx}.bin")
        # torch.save(model.state_dict(), model_save_path)
        # logging.info(f"Saved model parameters to {model_save_path}")
...
def load_data(cid, args):
    ...
    args.train_data_file = f'{args.train_data_dir}/split_{cid}.txt'
...
def load_model(args):
    ...
    if args.model_name_or_path:
        model = model_class.from_pretrained(args.model_name_or_path,
                                        ...
                                        config=config,
                                        cache_dir=args.cache_dir if args.cache_dir else None)  
        # file = "saved_models_uneven_nums/aggregated_parameters_round_2.pkl"
        #     # 加载 .pkl 文件中的模型参数
        # with open(file, 'rb') as f:
        #     parameters_obj = pickle.load(f)

        # # 确保 parameters_obj 是 Parameters 类型
        # if isinstance(parameters_obj, flwr.common.typing.Parameters):
        #     # print(1)
        #     # 将字节类型的张量转换为 PyTorch 张量
        #     print(parameters_obj)
        #     model_parameters = [torch.tensor(torch.load(io.BytesIO(tensor)), dtype=torch.float32) 
        #                         for tensor in parameters_obj.tensors]
        #     print(model_parameters)
        #     # 构建状态字典
        #     state_dict = {k: v for k, v in zip(model.state_dict().keys(), model_parameters)}
    ...
    model = Model(model,config,tokenizer,args).to(DEVICE)
